...
**本文件所在的 `east-china-sea-red-tide` 文件夹就是完整技能包，入口为 [SKILL.md](SKILL.md)。** 本地训练、推理和独立 Agent 验收已完成。不要只复制技能说明文件。

开源仓库：[Hu-yoouk/east-china-sea-red-tide](https://github.com/Hu-yoouk/east-china-sea-red-tide)。从仓库下载后，将 `skill/east-china-sea-red-tide/` 整个文件夹复制到目标 Agent 的技能目录。直接下载版本附件 ZIP 时，解压后即得到这个技能文件夹。

## 快速运行

安装 **Python 3.12**，在本文件所在的技能目录打开终端，先执行 `python --version` 确认版本；有多个 Python 时使用 3.12 的实际解释器路径。
...
将本技能文件夹整体放到平台支持的技能目录，注册为一个技能；不支持自动发现技能的平台，可以直接告诉 Agent：

> 请读取这个技能目录的 SKILL.md，按其规范校验我的数据，再调用真实程序预测并返回结果文件。不要用语言模型猜测概率。
...
完整技能包及 SHA256 校验文件见 [版本发布页](https://github.com/Hu-yoouk/east-china-sea-red-tide/releases/tag/v0.1.1)。代码沿用原项目木兰宽松许可证第 2 版，第三方数据与依赖授权单独处理；见 [材料与来源](docs/材料与来源.md)。验收边界见 [复现验收](docs/复现验收.md)，演示步骤见 [评委演示流程](docs/评委演示流程.md)。
